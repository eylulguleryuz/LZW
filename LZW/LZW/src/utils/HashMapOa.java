...
public class HashMapOa<K, V> implements EvaluableMap<K, V> {
    ...
    public HashMapOa(utils.HashManager.HashType ht) {
        this(DEFAULT_INITIAL_CAPACITY, ht);
    }

    public HashMapOa(int initialCapacity, utils.HashManager.HashType ht) {
    ...
    public V remove(K key) {
        ...
        node.value = get(key);
        int position = findPosition(key, false);
        table[position].key = DELETED.key;

        if (table[position].key == DELETED.key) table[position] = null; size--;
    ...
    public boolean replace(K key, V oldValue, V newValue) {
        ...
        for (Entry<K, V> node : table) {
            if(node != null && node.key == key && node.value.equals(oldValue)) {
                node.value = newValue;
                return true;
            }
        }
    ...
    public boolean containsValue(Object value) {
        if (value == null) {
            throw new IllegalArgumentException("Key is null in containsValue(Object value)");}
        for (Entry<K, V> node : table) {
